...
//            IndexResponse response= elasticsearchService.indexUsingElasticSearchClient(query);
    ...
    @RequestMapping(value="/searchByUsingClientWithFilter",method= RequestMethod.POST,produces = MediaType.APPLICATION_JSON_VALUE,consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Object> searchWithFilterByUsingClient(@RequestBody String query) {
        try {
    ...
    public ResponseEntity<Object> searchWithFilterByUsingRestAPI(@RequestBody String query) {
        try {
            ...
            String query2Str = "{\n" +
                    ...
                    "    ]\n" +
                    "}";
            String response= elasticsearchService.searchByRestAPIWithFilter(query,query2Str);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            return ResponseEntity.status(500).body("Error: " + e.getMessage());
        }
